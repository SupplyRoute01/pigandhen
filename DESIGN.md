...
- 상품명·가격·이미지는 제공된 상품 자료를 기준으로 표시하되, 고객 화면에는 자료 출처 문구를 노출하지 않는다. 연락처 등 확정되지 않은 정보만 임시 상태를 표시한다.
...
- 네이버 상품 썸네일은 원본 규격에 맞춘 1:1 정사각형으로 예약한다. 캠페인 컷과 세로 에디토리얼 컷은 콘텐츠 역할에 맞는 별도 비율을 유지한다.
- 영어는 짧은 카테고리·슬로건 레이블로만 사용하고 설명은 한국어로 제공한다.

## Layout and behavior

- 데스크톱 헤더는 흰 배경의 공식 워드마크와 중앙 내비게이션으로 구성한다.
- 제품 내비게이션은 데스크톱에서 호버·키보드 포커스로 열리는 2뎁스 메뉴, 모바일에서 탭으로 펼치는 아코디언 메뉴를 사용한다.
- 히어로는 밝은 회색 카피 영역과 캠페인 사진을 나란히 배치한다.
- 대표 카테고리는 사진 3열, 상품 카탈로그는 4열이며 760px 이하에서 두 열, 430px 이하에서 한 열로 축소한다.
- 상품이 많을 때는 처음 24개를 노출하고 명시적인 ‘상품 더 보기’ 버튼으로 24개씩 확장한다. 자바스크립트를 사용할 수 없을 때는 전체 상품을 그대로 노출한다.
- 상품은 시그니처, 로프, 비즈 & 체인, 뱅글, 여성, 주얼리 & 샤클 분류를 중복 소속할 수 있다. 각 세부 분류에서는 시그니처 상품을 먼저 배치한다.
- 자사 상품 상세 페이지가 마련되기 전까지 개별 상품 카드는 기존 상품 상세 페이지로 연결한다. 본사 소개 페이지나 외부 스토어 홈으로 이탈시키는 별도 홍보 버튼은 두지 않는다.
- 홈의 이야기 카드는 실제 글 상세 페이지로 연결하며, 상세 본문과 FAQ는 `story/posts.json`을 원본으로 사용한다. 상세 페이지는 질문형 제목, 넓은 본문 여백, 선형 구분으로 홈의 에디토리얼 언어를 이어간다.
